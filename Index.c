...
void desencriptadesplaza();
void desencriptapalabra();
...
{ char p_clave[256]="MmUuRrCcIiEeLlAaGgOo";//Entro directamente la palabra clave
   ...
   */
   ...
   desplazado=fopen("F_desplazado","rb"); //lo abre en modo lectura
   FILE *apc=NULL; //crea un puntero de archivo para el archivo "F_palabraclave"
   apc=fopen("F_palabraclave", "wb");  //Genera un nuevo archivo aunque exista
            ...
            { valor=fgetc(desplazado); //leo un caracter del archivo "F_desplazado"
              ...
              for(i=0;p_clave[i]!='\0'&& esta==0;i=i+1)
                 {if(p_clave[i]==valor)
                     ...
                     } //end if (p_clave[i]==valor)
                 } //end for(i=0;p_clave[i]!='\0'&& esta==0;i=i+1)
              if(esta==1)  //Si la bandera se puso en alto(cambio su valor igual a cero(0) 
                                    //al valor igual a uno(1)
                     fputc(j,apc);     //Grabo en "F_palabraclave" la posición(subíndice)  de "p_clave" del caracter
                                                //  leído de "F_desplazado"  
              else
...
int menu()
{
  ...
  printf("| 4. desncripta desplaza            |\n");
  printf("| 5. desencripta p.Clave            |\n");
  printf("|===================================|\n");
  printf("Selecione la opcion que desea ejecutar: ");
  scanf("%i", &opcion);
  while( opcion<0 || opcion>5 ) //Valido que entre un valor correcto entre [0 y 3]
       { printf( "\n Recuerda que solo se aceptan opciones numericas entre 0 y 3\n");
         printf("Selecione la opcion que desea ejecutar: ");
         scanf("%i", &opcion);
       } //end while(opcion<0||opcion>3)
...
void desencriptapalabra()
{
    ...
    char valor = 0;
    FILE *apc = NULL;
    apc = fopen("F_palabraclave", "rb");
    FILE *desencriptado = NULL;
    desencriptado = fopen("F_desencriptadopalabra.txt", "w");
    if (apc == NULL || desencriptado == NULL)
    {
        printf("\n Al abrir algún archivo se presentaron problemas.");
        getchar();
    }
    else
    {
        while (!feof(apc))
        {
            ...
            if (esta == 1)
                fputc(p_clave[j], desencriptado);
            else
                fputc(valor, desencriptado);
        }
        printf("\n Tamaño del archivo desencriptado: %li bytes", ftell(desencriptado));
        fclose(apc);
...
void desencriptadesplaza()
{
    int vd = 0;
    char valor = 0;
    FILE *desplazado = fopen("F_desplazado", "r");
    FILE *original = fopen("F_desencriptadodesplaza", "w");
    if (desplazado == NULL || original == NULL)
    {
        printf("\n Al abrir F_desplazado o crear el archivo F_desencriptadodesplaza.txt, se presentaron problemas ");
        getchar();
    }
    else
    {
